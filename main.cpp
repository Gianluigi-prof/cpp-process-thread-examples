...
int heap() {
    ...
    std::size_t size = 0.5 * 1024 * 1024 * 1024; // 1 GB in byte
...
{
    // Definiamo la dimensione in byte che vogliamo allocare
    std::size_t size = 10;//24 * 1024 * 1024; // 1 GB in byte

    try {
        // Allocazione dinamica di 1 GB di memoria
        char buffer  [size];

        std::cout << "Allocazione di " <<size <<" Byte di memoria avvenuta con successo." << std::endl;
        std::this_thread::sleep_for(std::chrono::minutes(10));
    } catch (std::bad_alloc& e) {
        std::cerr << "Errore di allocazione della memoria: " << e.what() << std::endl;
        return 1;
